...
export namespace DER {
  ...
  function resolve(opaque: Opaque): Triplet {
    if (opaque.type.inner.equals(Boolean.type))
      return opaque.into(Boolean.DER)
    if (opaque.type.inner.equals(Integer.type))
      return opaque.into(Integer.DER)
    if (opaque.type.inner.equals(BitString.type))
      return opaque.into(BitString.DER)
    if (opaque.type.inner.equals(OctetString.type))
      return opaque.into(OctetString.DER)
    if (opaque.type.inner.equals(Null.type))
      return opaque.into(Null.DER)
    if (opaque.type.inner.equals(ObjectIdentifier.type))
      return opaque.into(ObjectIdentifier.DER)
    if (opaque.type.inner.equals(UTF8String.type))
      return opaque.into(UTF8String.DER)
    if (opaque.type.inner.equals(PrintableString.type))
      return opaque.into(PrintableString.DER)
    if (opaque.type.inner.equals(Sequence.type))
      return resolveSequence(opaque.into(Sequence.DER))
    if (opaque.type.inner.equals(Set.type))
      return resolveSet(opaque.into(Set.DER))
    if (opaque.type.inner.equals(IA5String.type))
      return opaque.into(IA5String.DER)
    if (opaque.type.inner.equals(UTCTime.type))
      return opaque.into(UTCTime.DER)

    if (opaque.type.inner.clazz === Type.clazzes.UNIVERSAL)
      throw new Error(`Unknown UNIVERSAL type ${opaque.type.byte}`)

    if (opaque.type.inner.wrap === Type.wraps.CONSTRUCTED)
